# 📝 OCR - Extração Especializada de Textos e Tabelas em Mídias Sociais (Texto/Markdown): 3 Prompts - v1

# Nome: "ocr-extracao-especializada-textos-tabelas-em-midias-sociais-texto-markdown-3-prompts-v1.md"
...
# INSTRUÇÃO OCR - EXTRAÇÃO DE TEXTO DE IMAGENS

## OBJETIVO
Extrair todo o texto visível nesta imagem, respeitando a ordem natural de leitura (superior → inferior, esquerda → direita).

## CONTEXTO DA IMAGEM
- Verifique primeiro se é uma imagem de rede social (Instagram, Facebook, WhatsApp, Telegram)
- Se for rede social: ignore elementos de UI (ícones, botões, logos de aplicativo, contadores)
- Se NÃO for rede social: extraia todo texto visível

## REGRAS DE EXTRAÇÃO
1. Ordem de leitura: siga a sequência visual natural do texto.
2. Ortografia: mantenha exatamente como escrito (incluindo erros, maiúsculas, pontuação).
3. Formatação: texto puro, sem distinções de cor ou tamanho.
4. Quebras de linha: preserve quando houver separação clara entre blocos de texto.
5. Idioma: transcreva no idioma original (português, inglês, etc.).
6. Elementos a ignorar: logos, ícones de interface, marcas d'água de app, botões de navegação.

## FORMATO DE SAÍDA
Retorne apenas o texto extraído em formato markdown simples, sem comentários adicionais.

---

**IMAGEM ANEXADA:** [Cole ou anexe a imagem aqui]
...
## 📝 OCR - Extração Especializada de Tabelas em Mídias Sociais (Markdown):


````markdown
# INSTRUÇÃO OCR - TABELAS EM IMAGENS

## OBJETIVO
Ler o conteúdo desta imagem preservando ao máximo a estrutura de TABELAS e devolver o resultado em Markdown.

## TIPO DE IMAGEM
- A imagem é, principalmente, uma tabela ou uma grade com linhas e colunas.
- Pode ser print de rede social, planilha, painel, ou tabela inserida em um post.

## REGRAS GERAIS
1. Leia o conteúdo na ordem natural (de cima para baixo, da esquerda para a direita).
2. Mantenha a ortografia exatamente como aparece (incluindo erros e maiúsculas).
3. Ignore elementos de interface (botões, ícones, logos de app, barras de navegação, contadores de curtidas etc.).

## REGRAS ESPECÍFICAS PARA TABELAS
1. Identifique a área principal da tabela e considere-a prioridade.
2. Reconstrua cada tabela em **formato de tabela Markdown**:
   - Use `|` para separar colunas.
   - Use uma linha de cabeçalho (se existir) e uma linha de separador na segunda linha: `|---|---|---|`.
   - Cada linha visual da tabela deve virar **uma linha** na tabela Markdown.
3. Células:
   - Se uma célula estiver vazia, deixe o espaço vazio entre `| |`.
   - Não quebre números, datas ou palavras no meio de duas linhas; junte tudo em uma única célula.
4. Mesclagem de células:
   - Se uma célula estiver visualmente mesclada em mais de uma linha ou coluna, simplifique de forma legível:
     - Você pode repetir o valor nas células necessárias **ou**
     - Preencher apenas a célula “principal” e deixar as demais vazias, escolhendo o que ficar mais compreensível.
5. Se houver mais de uma tabela, separe-as com uma linha em branco entre cada tabela.

## FORMATO DE SAÍDA
- Retorne apenas Markdown, sem explicações ou comentários.
- Exemplo de estrutura esperada:

```markdown
| Coluna 1 | Coluna 2 | Coluna 3 |
|---------|---------|---------|
| linha1c1 | linha1c2 | linha1c3 |
| linha2c1 | linha2c2 | linha2c3 |
```

---

**IMAGEM ANEXADA (TABELA):** [Cole ou anexe a imagem aqui]
...
# INSTRUÇÃO OCR - IMAGENS DE MÍDIA SOCIAL (TEXTO + TABELAS)

## OBJETIVO
Extrair todo o texto visível desta imagem de mídia social, preservando a ordem de leitura e, quando existirem, as TABELAS em formato Markdown.

## IDENTIFICAÇÃO DA IMAGEM
1. Verifique se a imagem parece um print/captura de:
   - Instagram, Facebook, WhatsApp, Telegram ou outra rede social.

2. Se for rede social:
   - Ignore elementos de UI (botões, ícones, logos de app, barras de navegação, contadores, ícone de stories, etc.).

3. Se NÃO for rede social:
   - Trate como imagem genérica com texto e possíveis tabelas, seguindo as mesmas regras abaixo.
...
2. Ortografia:
   - Mantenha exatamente como aparece (incluindo erros, acentos, maiúsculas, pontuação).

3. Formatação:
   - Devolva apenas texto em Markdown simples (sem informações de cor, fonte ou tamanho).

4. Quebras de linha:
   - Use quebras de linha para separar blocos de texto claramente diferentes (título, subtítulo, corpo, rodapé).

5. Idioma:
   - Transcreva no(s) idioma(s) original(is) presentes na imagem.

## REGRAS ESPECÍFICAS PARA TABELAS
1. Detecte se existe alguma região que funcione como tabela ou grade (colunas alinhadas, linhas com múltiplas colunas, listagens tabulares).

2. Para cada tabela identificada:
   - Reconstrua em **tabela Markdown**:
     - Use `|` entre colunas.
     - Use uma linha de cabeçalho, se houver, seguida de uma linha de separador `|---|---|---|`.
     - Cada linha visual da tabela deve virar uma linha na tabela Markdown.

3. Células:
   - Mantenha o conteúdo completo de cada célula em uma única linha.
   - Se a célula estiver vazia, deixe o espaço vazio.

4. Mesclagem de células:
   - Simplifique para o formato Markdown mais legível:
     - Repita o conteúdo nas células correspondentes **ou**
     - Use apenas na célula principal e deixe as demais em branco, conforme ficar mais claro.

5. Caso haja mais de uma tabela:
   - Separe cada tabela com uma linha em branco no Markdown.

6. Texto ao redor da tabela:
   - Texto antes da tabela deve aparecer antes da tabela.
   - Texto depois da tabela deve aparecer em seguida, após a tabela.

## FORMATO DE SAÍDA
Retorne apenas o conteúdo extraído em Markdown, sem comentários adicionais, seguindo esta ideia de organização:

Texto inicial (títulos, descrição, etc.)
...
Texto final (observações, rodapé, etc.)

---

**IMAGEM ANEXADA (POST DE MÍDIA SOCIAL):** [Cole ou anexe a imagem aqui]
